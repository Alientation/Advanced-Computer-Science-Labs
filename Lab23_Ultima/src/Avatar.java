...
public class Avatar {
	...
	private int moveTickSpeed;					//ticks before move
	private int moveTickCount = 0;				//ticks since last move
	private Stats timer;                       // for timing display of hit points
	private Facing direction = Facing.DOWN;    // direction facing
	private boolean canMove = true;				//can the avatar move
	 ...
	 */
	public Avatar(int x, int y, int hp, int damage, double torch, int moveTickSpeed) {
		this.x = x;
		this.y = y;
		this.hp = hp;
		this.damage = damage;         
	...
	public void tick() {
		moveTickCount++;
		canMove = moveTickCount >= moveTickSpeed ? true : false;
		moveTickCount %= moveTickSpeed;
	}
	...
	public static void main(String [] args) {
		Avatar avatar = new Avatar(5, 5, 20, 4, 4.0, 6);
