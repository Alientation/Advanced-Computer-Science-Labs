...
class RecursionPractice11 {
   ...
   public static int makeChange (int amount) {
      return makeChangeHelper(amount,0,0,0,0,0, 4,false);
   }
   
   public static int makeChangeHelper(int amount, int currentAmount, int p, int n, int d, int q, int coinLeft, boolean doPrint) {
      ...
      int sum = 0;
      switch(coinLeft) {
         case 4: sum += makeChangeHelper(amount, currentAmount + 1, p + 1, n, d, q, 4,doPrint);
         case 3: sum += makeChangeHelper(amount, currentAmount + 5, p, n + 1, d, q, 3,doPrint);
         case 2: sum += makeChangeHelper(amount, currentAmount + 10, p, n, d + 1, q, 2,doPrint);
         case 1: sum += makeChangeHelper(amount, currentAmount + 25, p, n, d, q + 1, 1,doPrint);
      }
      return sum;
   }
   
   public static void makeChangeAndPrint (int amount) {
      makeChangeHelper(amount,0,0,0,0,0, 4,true);
   }
   /*
   public static int makeChangeHelper(int amount, int currentAmount, int p, int n, int d, int q, HashMap<String,Boolean> hs) {
      ...
      if (hs.get(p + " " + n + " " + d + " " + q) != null) {
         return 0;
      } else {
         hs.put(p + " " + n + " " + d + " " + q,false);
      }
