      ...
      int isBillFirst = 0;
		...
		while ((player1.size() < MAX_GEMS) && (player2.size() < MAX_GEMS))
		{
         ...
         if (turn1 == false) {
            ...
            int listToInsertAt = 2;
            
            int player2Score = player2.score();
            for (int i = 0; i <= player1.size(); i++) {
               player1.insertBefore(current,i);
               if (player2Score - player1.score() >= maxDifference) {
                  maxDifference = player2Score-player1.score();
                  indexToInsertAt = i;
                  listToInsertAt = 1;
               }
               player1.deleteAt(i);
            ...
            for (int i = 0; i <= player2.size(); i++) {
               player2.insertBefore(current,i);
               if (player2.score() - player1Score >= maxDifference) {
                  maxDifference = player2.score() - player1Score;
                  indexToInsertAt = i;
                  listToInsertAt = 2;
               }
               player2.deleteAt(i);
            }
            if (player2.size() == 15 && player2.score() > player1.score()) {
               listToInsertAt = 2;
               indexToInsertAt = 0;
            }
            if (listToInsertAt == 1) {
               player1.insertBefore(current,indexToInsertAt);
            } else {
               player2.insertBefore(current,indexToInsertAt);
            }
         ...
         */
         StdDraw.clear();
			...
			if (current == null) {
            if (isBillFirst != 0) {
               current = new Gem(Gem.pointValues[(int)(Math.random() * 7) + 1]);
            } else {
               current = new Gem();
            }
            if ((isBillFirst == 1 && turn1) || (isBillFirst == 2 && !turn1)) {
               current = new Gem(Gem.pointValues[Gem.pointValues.length - 1 - (int)(Math.random() * 6)]);
            }         
         }
